...
{
    // HDR should always occur in a floating-point framebuffer, so use gamma correction
...
LightResult CalculateLighting(float3 P, float3 N)
{
    ...
    // Do lighting in View space so that we do not have to pass Camera
    // Position to the shaders.
