...
SET input_main=..\..\src\Win32Main.cpp
SET output_main=/Fe:Reflection.exe
...
IF NOT EXIST bin\%mode%\CodeGen mkdir bin\%mode%\CodeGen

REM Copy test files, if they have changed.
echo.     -Copying Test Files:
if not exist bin\%mode%\Data mkdir bin\%mode%\Data
robocopy Data\ bin\%mode%\Data *.* /s /purge /ns /nc /ndl /np /njh /njs /mt
